...
window.addEventListener('DOMContentLoaded', function () {
  ...
  // Tab active효과
  const tabs = document.querySelectorAll('.tab_node');
  if (tabs) {
    tabs.forEach(function (tab, idx) {
      tab.addEventListener('click', function () {
        if (!this.classList.contains('is_active')) {
          const activeTab = this.parentNode.querySelector('.is_active');
          activeTab.classList.remove('is_active');
          this.classList.add('is_active');
        }
      })
    })
...
$('.close_summary').off('click').on('click', function () {
  $('.summary_news_container').removeClass('is_active');
